...
use tau_gui::chat::ChatState;
use tau_gui::view::{
    diff_reply_action, permission_reply_action, prompt_reply_action, stream_event_action,
    submit_action,
};
...
fn event(sequence: u64, value: TurnEvent) -> SequencedEvent {
    SequencedEvent {
...
fn scripted_daemon_replies_and_replay_use_typed_rpc_requests() {
    ...
    assert!(matches!(
        recorded.responses[0].response,
        ClientResponse::Permission { .. }
    ));
    assert!(matches!(
        recorded.responses[1].response,
        ClientResponse::Question { ref answer } if answer == "yes"
    ));
    assert!(matches!(
        recorded.responses[2].response,
...
fn typed_view_actions_apply_prompt_and_diff_replies_to_streamed_cards() {
    ...
    assert!(chat.reduce(submit_action("hello")));
    for (sequence, value) in [
        ...
        let _ = chat.reduce(stream_event_action(event(sequence, value)));
    }
    assert!(chat.reduce(permission_reply_action(
        2,
        tau_gui::chat::PermissionChoice::AllowOnce,
    )));
    assert!(chat.reduce(prompt_reply_action(2, "yes")));
    assert!(chat.reduce(diff_reply_action(3, true)));
