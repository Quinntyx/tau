...
use tau_gui::backend::{Backend, DaemonAction, TurnRequest};
use tau_gui::chat::{ChatAction, ChatState, PermissionChoice};
use tau_gui::picker::{PickerAction, fuzzy_models, next_agent};
...
fn scripted_daemon_replies_and_replay_use_typed_rpc_requests() {
    ...
    for response in [
        backend.respond(ClientResponse::Permission {
            request_id: "permission-1".into(),
            choice: TurnPermissionChoice::AllowOnce,
        }),
        backend.respond(ClientResponse::Question {
            question_id: "question-1".into(),
            answer: QuestionAnswer("yes".into()),
        }),
        backend.respond(ClientResponse::DiffHunk {
            request_id: "diff-1".into(),
            path: "src/lib.rs".into(),
            index: 0,
            approved: true,
    ...
    assert!(recorded.responses.iter().any(|request| matches!(
        request.response,
        ClientResponse::Question { ref answer, .. } if answer.0 == "yes"
    )));
    assert!(recorded.responses.iter().any(|request| matches!(
        request.response,
...
fn typed_view_actions_apply_prompt_and_diff_replies_to_streamed_cards() {
    ...
    let permission = chat
        .cards
        .iter()
        .position(|card| matches!(card, tau_gui::chat::Card::Permission { .. }))
        .unwrap();
    let question = chat
        .cards
        .iter()
        .position(|card| matches!(card, tau_gui::chat::Card::Question { .. }))
        .unwrap();
    let diff = chat
        .cards
        .iter()
        .position(|card| matches!(card, tau_gui::chat::Card::Diff { .. }))
        .unwrap();
    assert!(chat.reduce(ChatAction::Permission(
        permission,
        PermissionChoice::AllowOnce,
    )));
    assert!(chat.reduce(ChatAction::AnswerQuestion(question, "yes".into())));
    assert!(chat.reduce(ChatAction::ApproveDiff(diff, true)));
    assert!(
        chat.cards
            .iter()
            .any(|card| matches!(card, tau_gui::chat::Card::Question { answer: None, .. }))
    );
    assert!(chat.cards.iter().any(|card| matches!(
        card,
        tau_gui::chat::Card::Diff {
            approved: false,
            ..
        }
    )));
    assert!(matches!(
        chat.policy_states["permission-1"],
