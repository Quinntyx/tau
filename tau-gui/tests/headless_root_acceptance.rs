...
use tau_gui::shell::{
    ProjectItem, ProjectServiceAction, ProjectServiceAdapter, ProjectShell,
    ProjectState as ShellState, ShellAction, ShellLayout,
};
use tau_gui::{LocalProjectAdapter, ProjectShellRoot};
...
fn root_adapter_keeps_service_state_and_active_project_typed() {
    ...
    adapter.dispatch(ProjectServiceAction::Select(project_id.clone()));
    assert_eq!(
        adapter.state.active_project_id.as_deref(),
    ...
    adapter.dispatch(ProjectServiceAction::Update {
        id: project_id.clone(),
        name: "Renamed root".into(),
        root: PathBuf::from("/tmp/renamed-root"),
    });
    assert_eq!(
        adapter.state.active().map(|project| project.name.as_str()),
        Some("Renamed root")
    );

    // Keep the production root type in this acceptance target: the test is
    // intentionally window-free, but the adapter is the root's composition
    // boundary for project selection and repository-targeted actions.
