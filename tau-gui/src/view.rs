...
impl TauView {
    ...
    fn new_chat(&mut self, _: &MouseUpEvent, _: &mut Window, cx: &mut Context<Self>) {
        ...
        let project = ProjectId::new(self.backend.cwd().to_owned());
    ...
    fn session_project(&self) -> ProjectId {
        // S1 will replace this adapter with the registry's opaque ID.  Keeping
        // the conversion at this boundary avoids inventing project RPCs here.
        ProjectId::new(self.backend.cwd().to_owned())
    }

    fn load_sessions(&mut self, cx: &mut Context<Self>) {
        let backend = self.backend.clone();
        let project = self.session_project();
